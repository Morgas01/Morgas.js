(function(µ,SMOD,GMOD,HMOD,SC){
	...
	QUnit.module("util.object.iterate");
	...
	QUnit.test("iterate",function(assert)
	{
		...
		assert.deepEqual(arrCopy,arr,"array");
		...
		assert.propEqual(arrLikeCopy,[3,1,4],"array like");
		...
		assert.deepEqual(setCopy,[3,1,4,5,9,2,6,8,7,0],"Set");
		...
		assert.deepEqual(mapCopy,obj,"Map");
	});
	
	QUnit.test("Iterator",function(assert)
	{
		var it=Iterator([1]);
		assert.deepEqual(it.next().value,[0,1],"array 1");
		assert.ok(it.next().done===true,"array 2");
		
		it=Iterator({"myKey":"myValue"});
		assert.deepEqual(it.next().value,["myKey","myValue"],"object 1");
		assert.ok(it.next().done===true,"object 2");
	});
