(function(µ,SMOD,GMOD,HMOD,SC){
    ...
    dependencyParser.prototype.addProvidedModules=function(providedModules)
    {
    ...
    dependencyParser.prototype.parse=function(relativeTo)
    {
		...
		.then(parsedFiles=>
		{
			...
			for(var parsedFile of parsedFiles)
			{
				...
				var deps={
					deps:mapModuleToFile(parsedFile.deps,[rtn.moduleRegister,this.moduleRegister],relativeTo,this.providedModules),
					uses:mapModuleToFile(parsedFile.uses,[rtn.moduleRegister,this.moduleRegister],relativeTo,this.providedModules)
				}
			...
			{
				var deps={
					deps:mapModuleToFile(this.fileDependencies[file].deps,[rtn.moduleRegister,this.moduleRegister],relativeTo,this.providedModules),
					uses:mapModuleToFile(this.fileDependencies[file].uses,[rtn.moduleRegister,this.moduleRegister],relativeTo,this.providedModules)
				}
	...
	var mapModuleToFile=function (modules,registers,referencePath,providedModules)
	{
		...
		return SC.uniquify(modules.map(m=>
		{
			...
			if(!providedModules.has(m)) µ.logger.warn("module "+m+" not found");
			return false;
		})
